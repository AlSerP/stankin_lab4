...
GameField::GameField(string str_field) {
  this->field = Stack(int(str_field[0]) - int('0') - 1);

	for (int i = 0; i < 9; i++) {
		this->field += int(str_field[i]) - int('0') - 1;
    if (int(str_field[i]) - int('0') - 1 != -1)
      this->free_squares -= 1;
  }
}
...
void GameField::show_line(int line_num) {
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 3; j++) {
